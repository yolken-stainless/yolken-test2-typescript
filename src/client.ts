...
export interface ClientOptions {
   ...
   *
   * Defaults to process.env['YOLKEN_TEST2_BASE_URL'].
   */
   ...
   * Set the log level.
   *
   * Defaults to process.env['YOLKEN_TEST2_LOG'] or 'warn' if it isn't set.
   */
...
/**
 * API Client for interfacing with the Yolken Test2 API.
 */
export class YolkenTest2 {
  apiKey: string;
   ...
   * @param {string} [opts.baseURL=process.env['YOLKEN_TEST2_BASE_URL'] ?? https://petstore3.swagger.io/api/v3] - Override the default base URL for the API.
   ...
   */
  constructor({
    baseURL = readEnv('YOLKEN_TEST2_BASE_URL'),
    apiKey = readEnv('PETSTORE_API_KEY'),
    ...opts
  }: ClientOptions = {}) {
    if (apiKey === undefined) {
      throw new Errors.YolkenTest2Error(
        "The PETSTORE_API_KEY environment variable is missing or empty; either provide it, or instantiate the YolkenTest2 client with an apiKey option, like new YolkenTest2({ apiKey: 'My API Key' }).",
      );
    }
    ...
    this.timeout = options.timeout ?? YolkenTest2.DEFAULT_TIMEOUT /* 1 minute */;
    ...
    this.logLevel =
      ...
      parseLogLevel(readEnv('YOLKEN_TEST2_LOG'), "process.env['YOLKEN_TEST2_LOG']", this) ??
      defaultLogLevel;
  ...
  static YolkenTest2 = this;
  static DEFAULT_TIMEOUT = 60000; // 1 minute

  static YolkenTest2Error = Errors.YolkenTest2Error;
...
YolkenTest2.Pets = Pets;
YolkenTest2.Store = Store;
YolkenTest2.Users = Users;

export declare namespace YolkenTest2 {
